  ...
  update(updates) {
    for (let update of updates) {
      ...
      if (update.canvas) {
        ...
        let buffer = update.canvas;
        if (!(buffer instanceof Uint8ClampedArray)) {
          // TODO: this is a terrible hack for VS Code because VS Code only
          // supports passing JSON objects (not the structured clone algorithm).
          // Perhaps it would be better to convert to a binary string and pass
          // that along? Or just run the web worker inside the webview...
          buffer = new Uint8ClampedArray(Object.values(update.canvas));
        }
        let imageData = new ImageData(buffer, part.config.width, part.config.height);
